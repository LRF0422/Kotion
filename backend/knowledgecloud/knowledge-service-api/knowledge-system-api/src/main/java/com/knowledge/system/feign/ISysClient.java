...
import com.knowledge.core.tool.AppInfo;
...
public interface ISysClient {
	...
	DeptVO getDept(@RequestParam("id") Long id);
	...
	RoleFO getRole(@RequestParam("id") Long id);
	...
	R<TenantVO> getTenant(@RequestParam("id") Long id);
	...
	R<TenantVO> getTenant(@RequestParam("tenantId") String tenantId);

	@GetMapping(API_PREFIX + "/all")
	R<List<TenantVO>> getAllTenant();
	 ...
	 * @return
	 */
	@PostMapping(API_PREFIX + "/meta")
	R<?> registerMateData(@RequestBody AppInfo appInfo);
