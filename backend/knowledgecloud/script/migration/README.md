...
The wiki-domain tables (including `wiki_plugin`, `wiki_plugin_version`, `wiki_plugin_tag`) live in the `knowledge` schema, which is the database these migrations are applied to; connect with the default schema set to `knowledge` so the unqualified statements and the `DATABASE()` guards resolve to it.
