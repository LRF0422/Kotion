 ...
 */
package com.knowledge.core.swagger;
...
public class SwaggerAutoConfiguration {
	...
	public Docket api(SwaggerProperties swaggerProperties) {
		...
		return apis.build().securityContexts(securityContexts(swaggerProperties)).securitySchemes(securitySchemas(swaggerProperties))
			.extensions(openApiExtensionResolver.buildExtensions(knowledgeProperties.getName()));
	}
	...
	private List<SecurityReference> defaultAuth(SwaggerProperties swaggerProperties) {
		...
		if (securityReferenceList.size() == 0) {
			...
			securityReferenceList.add(new SecurityReference(SwaggerUtil.knowledgeAuth().getName(), authorizationScopes));
			securityReferenceList.add(new SecurityReference(SwaggerUtil.knowledgeTenant().getName(), authorizationScopes));
		}
	...
	private List<SecurityScheme> securitySchemas(SwaggerProperties swaggerProperties) {
		...
		if (swaggerApiKeyList.size() == 0) {
			return Lists.newArrayList(SwaggerUtil.clientInfo(), SwaggerUtil.knowledgeAuth(), SwaggerUtil.knowledgeTenant());
		} else {
