...
import com.knowledge.core.launch.utils.INetUtil;
...
public class KnowledgeApplication {
	...
	public static SpringApplicationBuilder createSpringApplicationBuilder(String appName, Class source,
		...
		String startJarPath = KnowledgeApplication.class.getResource("/").getPath().split("!")[0];
		String activePros = joinFun.apply(activeProfileList.toArray());
		printSystemConfig(appName, activePros, profile, startJarPath);
		...
		// NacosConstant.NACOS_CONFIG_PREFIX);
		props.setProperty("spring.cloud.nacos.config.file-extension", NacosConstant.NACOS_CONFIG_FORMAT);
		props.setProperty("spring.cloud.sentinel.transport.dashboard", SentinelConstant.SENTINEL_ADDR);
		props.setProperty("spring.cloud.alibaba.seata.tx-service-group",
				appName.concat(NacosConstant.NACOS_GROUP_SUFFIX));
		// 加载自定义组件
	 ...
	 * 输出系统配置信息
	 */
	private static void printSystemConfig(String appName, String activePros, String profile, String jarPath) {
		...
		String jvmVendor = System.getProperty("java.vm.vendor", "unknown");
		...
		String banner = "\n----------------------------------------------------------"
				+ "\n  Application  : " + appName.toUpperCase() + " (" + appName + ")"
				+ "\n  Version      : " + AppConstant.APPLICATION_VERSION
				+ "\n  Profile      : " + activePros
				+ "\n  Local        : " + isLocalDev()
				+ "\n  DevMode      : " + !AppConstant.PROD_CODE.equals(profile)
				+ "\n  Jar Path     : " + jarPath
				+ "\n  Host         : " + hostIp + " (" + hostName + ")"
				+ "\n----------------------------------------------------------"
				+ "\n  Java         : " + javaVersion + " (" + jvmName + ")"
