 ...
 */
package com.knowledge.core.cloud.version;

import feign.MethodMetadata;
import com.knowledge.core.cloud.annotation.ApiVersion;
import com.knowledge.core.cloud.annotation.UrlVersion;
...
/**
 * 支持 knowledge-boot 的 版本 处理
 *
 * @see com.knowledge.core.cloud.annotation.UrlVersion
 * @see com.knowledge.core.cloud.annotation.ApiVersion
 * @author L.cm
 */
public class KnowledgeSpringMvcContract extends SpringMvcContract {

	public KnowledgeSpringMvcContract(List<AnnotatedParameterProcessor> annotatedParameterProcessors, ConversionService conversionService) {
	...
	protected void processAnnotationOnMethod(MethodMetadata data, Annotation methodAnnotation, Method method) {
		if (RequestMapping.class.isInstance(methodAnnotation) || methodAnnotation.annotationType().isAnnotationPresent(RequestMapping.class)) {
			...
			if (apiVersion != null && StringUtil.isNotBlank(apiVersion.value())) {
				KnowledgeMediaType knowledgeMediaType = new KnowledgeMediaType(apiVersion.value());
				data.template().header(HttpHeaders.ACCEPT, knowledgeMediaType.toString());
			}
		}
	}
