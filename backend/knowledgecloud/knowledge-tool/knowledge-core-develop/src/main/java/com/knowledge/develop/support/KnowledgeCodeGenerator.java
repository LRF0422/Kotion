 ...
 */
package com.knowledge.develop.support;
...
import com.knowledge.develop.constant.DevelopConstant;
...
public class KnowledgeCodeGenerator {
	 ...
	 * 代码所在服务名
	 */
	private String serviceName = "knowledge-service";
	/**
	 * 代码生成的包名
	 */
	private String packageName = "com.knowledge.test";
	/**
	 * 代码后端生成的地址
	 */
	 ...
	 * 需要生成的表名(两者只能取其一)
	 */
	private String[] includeTables = {"knowledge_test"};
	/**
	 * 需要排除的表名(两者只能取其一)
	 */
	...
	public void run() {
		...
		FastAutoGenerator.create(url, username, password)
			...
			.strategyConfig(builder -> builder.addTablePrefix(tablePrefix).addInclude(includeTables).addExclude(excludeTables)
				.entityBuilder().naming(NamingStrategy.underline_to_camel).columnNaming(NamingStrategy.underline_to_camel).enableLombok().superClass("com.knowledge.core.mp.base.BaseEntity").addSuperEntityColumns(superEntityColumns).enableFileOverride()
				.serviceBuilder().superServiceClass("com.knowledge.core.mp.base.BaseService").superServiceImplClass("com.knowledge.core.mp.base.BaseServiceImpl").formatServiceFileName("I%sService").formatServiceImplFileName("%sServiceImpl").enableFileOverride()
				...
				.controllerBuilder().superClass("com.knowledge.core.boot.ctrl.knowledgeController").formatFileName("%sController").enableRestStyle().enableHyphenStyle().enableFileOverride()
			)
	...
	public String getOutputDir() {
		return (Func.isBlank(packageDir) ? System.getProperty("user.dir") + "/knowledge-ops/knowledge-develop" : packageDir) + "/src/main/java";
	}
