 ...
 */
package com.knowledge.core.datascope.handler;

import lombok.RequiredArgsConstructor;
import com.knowledge.core.datascope.enums.DataScopeEnum;
import com.knowledge.core.datascope.model.DataScopeModel;
...
import com.knowledge.core.tool.utils.PlaceholderUtil;
...
public class KnowledgeDataScopeHandler implements DataScopeHandler {

	private final ScopeModelHandler scopeModelHandler;

	@Override
	public String sqlCondition(String mapperId, DataScopeModel dataScope, KnowledgeUser knowledgeUser,
			String originalSql) {
		...
		DataScopeModel dataScopeDb = scopeModelHandler.getDataScopeByMapper(mapperId, knowledgeUser.getRoleId());
		...
		if (DataScopeEnum.ALL == scopeTypeEnum
				|| StringUtil.containsAny(knowledgeUser.getRoleName(), RoleConstant.ADMIN)) {
			return null;
		} else if (DataScopeEnum.CUSTOM == scopeTypeEnum) {
			whereSql = PlaceholderUtil.getDefaultResolver().resolveByMap(dataScope.getScopeValue(),
					BeanUtil.toMap(knowledgeUser));
		} else if (DataScopeEnum.OWN == scopeTypeEnum) {
			ids.add(knowledgeUser.getUserId());
		} else if (DataScopeEnum.OWN_DEPT == scopeTypeEnum) {
			ids.addAll(Func.toLongList(knowledgeUser.getDeptId()));
		} else if (DataScopeEnum.OWN_DEPT_CHILD == scopeTypeEnum) {
			List<Long> deptIds = Func.toLongList(knowledgeUser.getDeptId());
			ids.addAll(deptIds);
		...
		return StringUtil.format("select {} from ({}) scope " + whereSql, Func.toStr(dataScope.getScopeField(), "*"),
				originalSql, dataScope.getScopeColumn(), StringUtil.join(ids));
	}
