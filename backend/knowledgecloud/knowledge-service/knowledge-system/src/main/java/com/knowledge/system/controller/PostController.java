...
import com.knowledge.system.domain.Post;
import com.knowledge.system.service.IPostService;
import com.knowledge.system.vo.PostVO;
import com.knowledge.system.wrapper.PostWrapper;
...
public class PostController extends KnowledgeController {
	...
	public R submit(@Valid @RequestBody Post post) {
		post.setTenantId(SecurityContextUtil.getTenantId());
	...
	public R remove(@ApiParam(value = "主键集合", required = true) @RequestParam String ids) {
		return R.status(postService.removeBatchByIds(Func.toLongList(ids)));
	}
	...
	public R<List<Post>> select(String tenantId, KnowledgeUser knowledgeUser) {
		List<Post> list = postService.list(Wrappers.<Post>query().lambda().eq(Post::getTenantId,
				Func.toStr(tenantId, knowledgeUser.getTenantId())));
		return R.data(list);
	}
