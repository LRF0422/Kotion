...
export const TableViewCfg = (props: { visible: boolean, toggle: () => void }) => {
    ...
    return <div
        className={cn("absolute p-1 inset-y-0 right-0 w-[250px] border rounded-sm  bg-popover text-popover-foreground z-50 text-sm shadow-md ", visible ? " slide-in-from-right animate-in transition-all duration-500 visible" : " invisible slide-out-to-right transition-all duration-500  animate-out opacity-0")}
    >
                    ...
                    <div className=" text-sm flex flex-row items-center justify-between gap-1">
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent side="right" align="start" className="w-[150px]">
                                ...
                                {
                                    columnTypes && columnTypes.map((type: any, index: number) => (
