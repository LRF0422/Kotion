 ...
 *
 * Values come from SheetJS' formatted output, so dates and number formats look
 * the same as they do in Excel. Formulas are preserved as their `=…` text.
 */
export async function parseExcelToWorkbook(file: File, workbookId?: string): Promise<WorkbookData> {
    ...
    try {
        book = XLSX.read(arrayBuffer, { type: 'array', cellFormula: true, cellText: true })
    } catch (error) {
        logger.warn('[office/spreadsheet] XLSX.read failed', error)
        throw new Error(`无法解析文件「${file.name}」，请确认它是有效的 Excel/CSV 文件`)
    }

    const sheetNames = book.SheetNames?.filter((name) => !!book.Sheets[name]) ?? []
    ...
    const sheets = sheetNames.slice(0, 20).map((sheetName, index) => {
