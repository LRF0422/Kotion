...
export function evaluateExpression(
  ...
  try {
    ...
    if (splitAtEq[splitAtEq.length - 1]!.length > 0) {
      changedLatex = splitAtEq[splitAtEq.length - 1] as string;
    } else if (splitAtEq.length >= 2) {
      changedLatex = splitAtEq[splitAtEq.length - 2] as string;
    }
    ...
    for (const id in variables) {
      const variable: MathVariable = variables[id] as MathVariable;
