...
export const FileCard: React.FC<FileCardProps> = (props) => {
    const { name, isFolder, objectId } = props
        ...
        <CardHeader className="p-0">
            <CardTitle className="p-0 m-0">
                <div className="flex items-center justify-between p-1">
                    <Checkbox className="ml-1" defaultChecked={checked} onCheckedChange={(checked) => {
...
export const FileCardList: React.FC<FileCardListPros> = (props) => {
    const { files, selectedFiles, setSelectFiles } = props
    return <div className="flex flex-wrap w-full gap-2 p-6 overflow-auto">
        {
            files.map((it, index) => (
                <FileCard
                    onSelect={(id, checked) => {
