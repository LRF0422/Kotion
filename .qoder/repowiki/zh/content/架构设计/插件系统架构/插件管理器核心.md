...
## 更新摘要
**变更内容**
- 新增 loadExternalPluginExtensions 方法的详细说明，支持外部插件扩展加载
- 补充协作场景下的插件扩展加载机制
- 更新插件管理器架构图以反映新的外部插件加载能力
- 增强插件状态管理和事件通知机制的说明
- 修正 loadExternalPluginExtensions 方法中的语法错误
...
- 插件实例创建过程与错误处理
- 插件状态管理与事件通知机制
- **新增**：外部插件扩展加载机制（loadExternalPluginExtensions）
- 最佳实践与性能优化建议
...
subgraph "协作场景"
CollabDoc["COLLABORATION_API.md"]
end
PM --> ImportUtil
PM --> Event
PM --> Types
...
**图表来源**
- [packages/common/src/core/PluginManager.ts](file://packages/common/src/core/PluginManager.ts#L99-L559)
...
- [packages/common/src/core/editor.ts](file://packages/common/src/core/editor.ts#L1-L41)
...
- [packages/plugin-main/src/index.tsx](file://packages/plugin-main/src/index.tsx#L1-L356)
...
- **新增**：ExtensionWrapper：编辑器扩展包装器，定义扩展的结构和配置。
...
- [packages/common/src/utils/import-util.ts](file://packages/common/src/utils/import-util.ts#L1-L39)
- [packages/common/src/event/event.ts](file://packages/common/src/event/event.ts#L1-L44)
- [packages/common/src/core/editor.ts](file://packages/common/src/core/editor.ts#L6-L41)

## 架构总览
下图展示插件管理器在系统中的角色与交互，包括新增的外部插件扩展加载能力：
...
- 职责边界
  ...
  - **新增**：外部插件扩展加载：支持在不改变当前用户插件配置的情况下加载其他用户的插件扩展。
...
+resolvePluginSettings() : PluginSettingsConfig[]
...
PluginManager --> ExtensionWrapper : "解析扩展"
```
...
**图表来源**
- [packages/common/src/core/PluginManager.ts](file://packages/common/src/core/PluginManager.ts#L151-L220)
- [packages/common/src/utils/import-util.ts](file://packages/common/src/utils/import-util.ts#L1-L39)
...
- [packages/core/src/components/Shop/Marketplace/index.tsx](file://packages/core/src/components/Shop/Marketplace/index.tsx#L33-L61)
- [packages/core/src/components/Shop/index.tsx](file://packages/core/src/components/Shop/index.tsx#L110-L210)
- [packages/common/src/core/PluginManager.ts](file://packages/common/src/core/PluginManager.ts#L252-L292)
- [packages/common/src/utils/import-util.ts](file://packages/common/src/utils/import-util.ts#L1-L39)
...
### 外部插件扩展加载机制
- **新增功能**：loadExternalPluginExtensions 方法
- **设计目的**：在协作场景中加载其他用户的插件扩展而不影响当前用户的插件配置
- **核心特性**：
  - 接受插件元数据数组（包含 resourcePath、pluginKey、name）
  - 使用与 init 方法相同的 URL 构建逻辑
  - 仅提取 editorExtensions 而不修改当前插件列表
  - 返回 ExtensionWrapper 数组供编辑器使用

- **工作流程**
  - 验证插件元数据的有效性
  - 构建插件 URL（支持 HTTP 和相对路径）
  - 使用 importScript 动态加载插件脚本
  - 提取 KPlugin 实例并收集 editorExtensions
  - 返回所有成功加载插件的编辑器扩展

- **错误处理**
  - 使用 Promise.allSettled 处理部分加载失败的情况
  - 记录警告日志但不影响其他插件的加载
  - 返回已成功加载的扩展数组

```mermaid
sequenceDiagram
participant CM as "协作场景"
participant PM as "PluginManager"
participant Loader as "importScript"
participant Ext as "ExtensionWrapper[]"
CM->>PM : "loadExternalPluginExtensions(plugins)"
PM->>PM : "验证插件元数据"
PM->>PM : "构建插件URL"
PM->>Loader : "加载插件脚本"
Loader-->>PM : "返回插件实例"
PM->>PM : "提取 editorExtensions"
PM->>Ext : "收集扩展"
PM-->>CM : "返回扩展数组"
```

**图表来源**
- [packages/common/src/core/PluginManager.ts](file://packages/common/src/core/PluginManager.ts#L493-L537)
...
**图表来源**
- [packages/common/src/core/PluginManager.ts](file://packages/common/src/core/PluginManager.ts#L222-L230)
- [packages/common/src/core/PluginManager.ts](file://packages/common/src/core/PluginManager.ts#L539-L541)
- [packages/common/src/core/types.ts](file://packages/common/src/core/types.ts#L1-L4)
- [packages/core/src/hooks/use-service.ts](file://packages/core/src/hooks/use-service.ts#L1-L15)
...
- 错误处理
  ...
  - **新增**：loadExternalPluginExtensions 使用 Promise.allSettled，即使部分插件加载失败也不会影响整体功能。
...
- 状态字段
  ...
  - **新增**：_pluginMap：插件名称到插件实例的映射表，提高查找效率。
...
- **新增**：外部插件扩展加载状态
  - loadExternalPluginExtensions 不影响插件管理器的内部状态
  - 仅临时提供编辑器扩展功能，适合一次性使用场景

**章节来源**
- [packages/common/src/core/PluginManager.ts](file://packages/common/src/core/PluginManager.ts#L107-L118)
- [packages/common/src/event/event.ts](file://packages/common/src/event/event.ts#L1-L44)
- [packages/core/src/App.tsx](file://packages/core/src/App.tsx#L158-L187)
- [packages/core/src/components/Shop/Marketplace/index.tsx](file://packages/core/src/components/Shop/Marketplace/index.tsx#L33-L61)
- [packages/core/src/components/Shop/index.tsx](file://packages/core/src/components/Shop/index.tsx#L110-L210)
...
- 组件耦合
  ...
  - **新增**：编辑器扩展依赖 ExtensionWrapper 类型定义。

- 外部依赖
  - lodash.merge：用于服务合并。
  - 浏览器动态脚本加载：document.createElement/script。
  - **新增**：@tiptap/core：编辑器扩展的基础类型定义。
...
- 并行加载
  ...
  - **新增**：loadExternalPluginExtensions 使用 Promise.allSettled，即使部分插件加载失败也能快速返回结果。
- 脚本缓存
  - importScript 内置缓存，避免重复下载同一 URL。
- 服务合并成本
...
- **新增**：插件映射优化
  - 使用 Map 数据结构存储插件映射，提高插件查找效率。
  - 缓存解析结果（路由、菜单、扩展、多语言），避免重复计算。
...
- **新增**：外部插件扩展加载问题
  - 现象：loadExternalPluginExtensions 返回空数组或部分扩展缺失。
  - 排查：检查插件元数据的 resourcePath、pluginKey 是否正确，确认插件是否包含 editorExtensions。
  - 建议：查看日志输出，确认哪些插件加载失败以及失败原因。
...
## 结论
PluginManager 通过清晰的职责划分与简洁的 API 设计，实现了插件的动态装载、服务合并与 UI 事件联动。其并行加载与脚本缓存提升了性能，而事件驱动的刷新机制保证了 UI 与插件状态的一致性。**新增的 loadExternalPluginExtensions 方法**进一步增强了协作场景下的插件扩展加载能力，使得系统能够在不改变用户现有插件配置的情况下，灵活地集成其他用户的插件功能。建议在实际工程中补充错误处理与性能监控，以进一步提升稳定性与可观测性。
...
- 最佳实践
  ...
  - **新增**：合理使用 loadExternalPluginExtensions，在协作场景中临时加载插件扩展。
- 参考实现
  - 插件示例：plugin-main、plugin-ai、plugin-database 提供了路由、菜单、编辑器扩展与服务的完整范式。
  - **新增**：协作场景文档：COLLABORATION_API.md 详细描述了外部插件扩展加载的完整流程。