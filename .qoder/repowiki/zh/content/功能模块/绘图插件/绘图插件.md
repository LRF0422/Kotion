...
- Drawnix 白板：基于 Plait 的现代化思维导图与白板系统，支持思维导图模式和白板模式，内置擦除器和自由手绘工具，提供丰富的绘图工具集。
...
- 插件扩展 extension/plugins/*：提供特定功能的插件模块（如擦除器、自由手绘）。
...
NPlugins["plugin-drawnix/src/extension/plugins/*"]
NSkills["plugin-drawnix/src/extension/skills/drawnix-skill.ts"]
end
...
- 插件注册与扩展注入
  ...
  - Drawnix 插件现在支持两种模式：思维导图模式和白板模式，默认为白板模式。
  - 参考路径：
    - [packages/plugin-drawnix/src/index.tsx:1-14](file://packages/plugin-drawnix/src/index.tsx#L1-L14)
    - [packages/plugin-drawnix/src/extension/index.tsx:38-50](file://packages/plugin-drawnix/src/extension/index.tsx#L38-L50)

- 节点与命令
  - 各插件扩展均通过 Node.create 定义节点类型、属性、渲染与 NodeView，并提供 insert* 命令用于插入对应节点。
  - Drawnix 节点现在支持 mode 属性，用于区分思维导图模式和白板模式。
  - 参考路径：
...
- 视图渲染与交互
  ...
  - Drawnix 视图现在支持动态插件加载，根据模式自动启用相应的功能插件。
  - 参考路径：
    - [packages/plugin-drawnix/src/extension/DrawnixView.tsx:82-117](file://packages/plugin-drawnix/src/extension/DrawnixView.tsx#L82-L117)
...
**图表来源**
- [packages/plugin-drawnix/src/extension/drawnix.ts:183-237](file://packages/plugin-drawnix/src/extension/drawnix.ts#L183-L237)
- [packages/plugin-drawnix/src/extension/DrawnixView.tsx:96-101](file://packages/plugin-drawnix/src/extension/DrawnixView.tsx#L96-L101)
...
**章节来源**
- [packages/plugin-drawio/src/extension/drawio.ts:22-79](file://packages/plugin-drawio/src/extension/drawio.ts#L22-L79)
- [packages/plugin-drawio/src/extension/DrawioView.tsx:1-90](file://packages/plugin-drawio/src/extension/DrawioView.tsx#L1-L90)

### Drawnix 白板
- 实现特点
  - 基于 Plait 的现代化思维导图与白板系统，支持思维导图模式和白板模式，默认为白板模式。
  - 白板模式下提供完整的绘图工具集：手型工具、选择工具、矩形、椭圆、菱形、文本、箭头、画笔、擦除器、思维导图工具。
  - 支持动态插件加载：思维导图模式加载 withDraw、withGroup、withMind、withMindExtend；白板模式额外加载 withFreehand、withFreehandCreate、withEraser。
  - 内置擦除器插件，支持点击删除元素；自由手绘插件支持流畅的手绘线条。
- 适用场景
  - 需要多功能绘图工具的场景，既支持思维导图也支持白板创作。
- 集成方式
  - 通过 insertDrawnix 命令插入节点，默认为白板模式；支持通过 mode 属性切换模式。
  - 提供丰富的工具集和键盘快捷键支持。
- 数据存储策略
...
class EraserPlugin {
+ERASER_TYPE : string
+withEraser(board) : PlaitPlugin
}
class FreehandPlugin {
+FREEHAND_TYPE : string
+withFreehand(board) : PlaitPlugin
+withFreehandCreate(board) : PlaitPlugin
}
DrawnixView --> Drawnix : "使用节点属性/命令"
DrawnixView --> EraserPlugin : "动态加载"
DrawnixView --> FreehandPlugin : "动态加载"
```

**图表来源**
- [packages/plugin-drawnix/src/extension/drawnix.ts:183-237](file://packages/plugin-drawnix/src/extension/drawnix.ts#L183-L237)
- [packages/plugin-drawnix/src/extension/DrawnixView.tsx:96-101](file://packages/plugin-drawnix/src/extension/DrawnixView.tsx#L96-L101)
- [packages/plugin-drawnix/src/extension/plugins/with-eraser.ts:11-41](file://packages/plugin-drawnix/src/extension/plugins/with-eraser.ts#L11-L41)
- [packages/plugin-drawnix/src/extension/plugins/with-freehand.ts:15-31](file://packages/plugin-drawnix/src/extension/plugins/with-freehand.ts#L15-L31)
...
**章节来源**
- [packages/plugin-excalidraw/src/excalidraw/excalidraw.ts:14-67](file://packages/plugin-excalidraw/src/excalidraw/excalidraw.ts#L14-L67)
- [packages/plugin-excalidraw/src/excalidraw/ExcalidrawView.tsx:1-53](file://packages/plugin-excalidraw/src/excalidraw/ExcalidrawView.tsx#L1-L53)
...
**章节来源**
- [packages/plugin-mermaid/src/editor-extension/mermaid/mermaid.ts:13-43](file://packages/plugin-mermaid/src/editor-extension/mermaid/mermaid.ts#L13-L43)
...
- 外部依赖
  ...
  - Drawnix：@plait/* 生态组件，支持思维导图和白板功能，依赖主题与视口适配。
...
- 内部依赖
  ...
  - Drawnix 插件依赖 @plait-board/react-board 和相关插件生态系统。

```mermaid
graph LR
Drawio["@kn/editor<br/>DrawioView"] --> Diag["diagrams.net iframe"]
Drawnix["@kn/editor<br/>DrawnixView"] --> Plait["@plait/*<br/>React Board"]
Drawnix --> Plugins["擦除器/自由手绘插件"]
...
- Drawnix
  ...
  - 插件动态加载机制会在模式切换时重新初始化，注意性能影响。
  - 擦除器和自由手绘插件可能增加渲染复杂度，建议在大量元素时谨慎使用。
- Excalidraw
...
- Drawnix
  ...
  - 现象：擦除器或自由手绘工具不工作
  - 排查：确认插件是否正确加载，检查 isWhiteboard 模式判断，验证插件初始化顺序。
  - 现象：工具切换异常或键盘快捷键失效
  - 排查：检查 switchTool 函数，确认 pointer 类型映射和 creation mode 设置。
- Excalidraw
  - 现象：渲染异常或库变更未生效
...
**章节来源**
- [packages/plugin-drawnix/src/extension/DrawnixView.tsx:155-179](file://packages/plugin-drawnix/src/extension/DrawnixView.tsx#L155-L179)
- [packages/plugin-drawnix/src/extension/plugins/with-eraser.ts:17-38](file://packages/plugin-drawnix/src/extension/plugins/with-eraser.ts#L17-L38)
- [packages/plugin-drawnix/src/extension/plugins/with-freehand.ts:115-159](file://packages/plugin-drawnix/src/extension/plugins/with-freehand.ts#L115-L159)

## 结论
本插件体系以扩展、视图和插件分离的设计，兼顾了专业性与易用性：
- Draw.io 适合复杂图表与外部协作；
- Drawnix 现已发展为多功能绘图系统，支持思维导图和白板模式，内置擦除器和自由手绘工具；
- Excalidraw 适合轻量手绘与团队草图；
- Mermaid 适合文本驱动的图表创作；
- Mindmap Canvas 适合轻量级思维导图。

通过统一的 NodeViewWrapper 与 updateAttributes 机制，所有绘图数据均可持久化并融入编辑器生态。

## 附录
- 使用示例与集成步骤（以 Drawnix 为例）
  - 在编辑器中调用 insertDrawnix 命令插入节点，默认为白板模式。
  - 在视图中使用工具栏或键盘快捷键切换工具，支持擦除器和自由手绘。
  - 数据自动写回节点属性，可在其他页面或服务端读取。
  - 参考路径：
    - [packages/plugin-drawnix/src/extension/drawnix.ts:205-214](file://packages/plugin-drawnix/src/extension/drawnix.ts#L205-L214)
    - [packages/plugin-drawnix/src/extension/DrawnixView.tsx:248-259](file://packages/plugin-drawnix/src/extension/DrawnixView.tsx#L248-L259)
- 数据导入导出最佳实践
  - Draw.io：通过 export 返回的 XML PNG 进行导入/导出；在服务端可转换为图片或 SVG。
  - Drawnix：支持思维导图模式的数据导入导出，白板模式支持完整的绘图数据持久化。
  - Excalidraw/Mermaid/Mindmap Canvas：直接读取节点属性 data/elements/appState/files/libraryItems 进行持久化或迁移。
- 协作编辑建议
  - 对于需要实时协作的场景，优先考虑 Excalidraw 的协作能力；对于只读展示，可将其置于只读模式。
  - Drawnix 的思维导图模式适合团队协作编辑；白板模式更适合个人创作。
