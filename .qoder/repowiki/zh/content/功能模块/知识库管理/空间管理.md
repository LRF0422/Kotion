...
## 更新摘要
**变更内容**
- 更新空间创建表单章节，反映从简单表单升级为多段式表单的完整实现
- 新增实时验证、帮助文本、封面图片选择系统的详细说明
- 补充提交反馈机制和国际化支持的完整实现
- 更新架构图以反映新的表单结构和验证流程
- **更新验证错误消息属性**：空间表单组件的验证错误消息属性从 `message` 更新为 `error`，以适配新的 Zod 版本
...
**更新** 本版本重点反映了空间创建表单的重大升级，从简单的单段式表单演进为包含基本资料和外观两个独立部分的多段式表单，具备完整的实时验证、帮助文本、封面图片选择系统和提交反馈机制。同时，验证错误消息属性已更新为适配新的 Zod 版本。
...
subgraph "组件层"
SpaceForm["SpaceForm 表单<br/>多段式创建表单"]
...
- [packages/plugin-main/src/pages/components/SpaceForm.tsx](file://packages/plugin-main/src/pages/components/SpaceForm.tsx#L1-L309)
...
**更新** 空间创建表单现已升级为多段式表单，包含基本资料和外观两个独立部分，每个部分都具备完整的验证和帮助文本支持。验证错误消息属性已更新为 `error` 以适配新的 Zod 版本。
...
### 创建空间（多段式表单与服务调用）
- 多段式表单组件 SpaceForm
  - **基本资料部分**：包含空间名称和描述字段，具备实时验证和帮助文本。
  - **外观部分**：包含图标选择和封面图片选择系统，支持上传和预设选项。
  - **实时验证**：使用 Zod 进行字段验证，提供即时反馈。**更新**：验证错误消息属性已更新为 `error` 以适配新的 Zod 版本。
  - **帮助文本**：每个字段都有对应的说明文本，提升用户体验。
  - **封面图片系统**：支持上传自定义封面和选择预设封面。
  - **提交反馈**：包含加载状态和成功/失败提示。
- 服务层对接
  - spaceService 本身不直接暴露创建空间方法，通常由页面组件直接调用 APIS.CREATE_SPACE 完成创建。

**更新** 空间创建表单现已升级为完整的多段式表单，具备以下特性：
- **分段设计**：将表单分为基本资料和外观两个独立部分，提升用户体验
- **实时验证**：使用 Zod Schema 进行字段验证，提供即时反馈
- **帮助文本**：每个字段都有详细的说明文本
- **封面系统**：支持上传自定义封面和选择预设封面
- **提交反馈**：完整的加载状态和错误处理机制
- **验证错误消息属性更新**：已从 `message` 更新为 `error` 以适配新的 Zod 版本
...
**图表来源**
- [packages/plugin-main/src/pages/components/SpaceForm.tsx](file://packages/plugin-main/src/pages/components/SpaceForm.tsx#L1-L309)
- [packages/plugin-main/src/api/index.ts](file://packages/plugin-main/src/api/index.ts#L48-L51)
...
- **新增多段式表单国际化支持**：包含基本资料、外观、图标、封面等完整的翻译键。

**更新** 国际化系统现已支持多段式表单的所有界面元素，包括：
- 表单段落标题：`creation.section-basic`、`creation.section-appearance`
- 字段标签和占位符：`creation.name`、`creation.desc`、`creation.icon`、`creation.cover`
- 帮助文本：`creation.name-help`、`creation.desc-help`、`creation.icon-help`、`creation.cover-help`
- 操作按钮：`creation.submit`、`creation.cancel`、`creation.creating`
- **验证错误消息**：`creation.name-required`（已更新为 error 属性）
- 成功/失败消息：`creation.success`、`creation.error`、`creation.retry`

**章节来源**
- [packages/plugin-main/src/index.tsx](file://packages/plugin-main/src/index.tsx#L98-L116)
- [packages/plugin-main/src/index.tsx](file://packages/plugin-main/src/index.tsx#L220-L238)
...
- 图片与封面
  ...
  - **多段式表单优化**：封面图片采用预览和懒加载机制，减少不必要的网络请求。
- 事件与状态
  - 合理使用事件订阅与解绑，避免重复监听导致的内存泄漏。
  - 对频繁刷新的状态（如收藏/刷新标志）进行最小化更新，减少重渲染。
  - **实时验证优化**：使用去抖机制避免频繁的验证请求。
  - **验证错误消息优化**：使用 `error` 属性的验证消息格式，提升验证性能。
...
- 常见问题
  ...
  - **多段式表单问题**：
    - 字段验证失败：检查 Zod Schema 配置和翻译键是否正确。**更新**：确保验证错误消息使用 `error` 属性而非 `message`。
    - 封面上传失败：确认上传组件和文件服务配置。
    - 提交反馈异常：检查 toast 组件和错误处理逻辑。
  - 邀请失败：检查邮箱格式与权限数组，确认 API 返回状态码与提示。
- 建议
  - 在 useApi 调用处增加 try/catch 与错误分支处理，统一提示。
  - 对收藏/模板/回收站等高频操作增加本地状态反馈与重试机制。
  - 对于权限不足场景，提供明确的引导与降级 UI。
  - **多段式表单调试**：利用实时验证反馈快速定位问题字段。
  - **验证错误消息调试**：如果验证消息不显示，检查是否使用了正确的 `error` 属性格式。

**更新** 新增多段式表单相关的故障排查指导，特别是验证错误消息属性的调试方法。
...
## 结论
本方案以清晰的服务层抽象为核心，结合页面与组件实现，完成了从首页入口到空间中心的全链路空间管理能力。通过统一的 API 常量与服务接口，既保证了可维护性，也为后续扩展（权限、模板、协作）提供了良好基座。

**更新** 本次重大升级将空间创建表单从简单的单段式表单演进为功能完整的多段式表单，显著提升了用户体验和表单的易用性。新的表单架构具备：
- 清晰的信息分组（基本资料 vs 外观设置）
- 完整的实时验证和反馈机制
- 丰富的帮助文本和用户指导
- 灵活的封面图片选择系统
- 优雅的国际化支持
- **适配新的 Zod 版本**：验证错误消息属性已更新为 `error` 以确保兼容性和正确性

建议在权限控制与错误处理方面进一步完善，以提升稳定性与用户体验。
...
### 多段式表单国际化键值
- 基本信息段落：`creation.section-basic`、`creation.section-basic-desc`
- 外观段落：`creation.section-appearance`、`creation.section-appearance-desc`
- 字段标签：`creation.name`、`creation.desc`、`creation.icon`、`creation.cover`
- 帮助文本：`creation.name-help`、`creation.desc-help`、`creation.icon-help`、`creation.cover-help`
- 操作按钮：`creation.submit`、`creation.cancel`、`creation.creating`
- 成功/失败消息：`creation.success`、`creation.error`、`creation.retry`
- **验证错误消息**：`creation.name-required`（使用 error 属性）

**更新** 新增多段式表单相关的完整国际化键值，包括验证错误消息的属性说明。
...
### 验证错误消息属性更新说明
**更新** 验证错误消息属性已从 `message` 更新为 `error`：

- **旧版本（不推荐）**：
  ```typescript
  name: z.string({
      message: t("creation.name-required", "Space name is required")
  }).min(1, {
      message: t("creation.name-required", "Space name is required")
  })
  ```
...
这种更新确保了与最新 Zod 版本的兼容性，并提供了更清晰的错误消息结构。

**章节来源**
- [packages/plugin-main/src/pages/components/SpaceForm.tsx](file://packages/plugin-main/src/pages/components/SpaceForm.tsx#L35-L36)