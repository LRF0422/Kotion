...
## 更新摘要
**所做更改**
- 新增未知内容重写系统章节，详细说明null安全检查和不受支持内容类型的回退机制
- 更新协作编辑器和渲染器章节，增加rewriteUnknownContent函数的使用说明
- 新增未知节点扩展和视图组件的详细说明
- 增强系统稳定性与可靠性章节，突出null安全检查的重要性
...
- 未知内容重写系统：null安全检查与不受支持内容类型的回退机制
- 扩展开发指南：自定义扩展的创建、注册与配置
- 具体示例路径：如何实现自定义编辑器功能与集成第三方扩展

## 项目结构
编辑器核心位于 packages/editor，围绕 Tiptap/ProseMirror 构建，通过扩展系统与插件体系实现可插拔能力；协作能力由 Hocuspocus 提供者与 Yjs 文档驱动。新增的未知内容重写系统为编辑器提供了强大的内容兼容性保障。
...
REWRITE["未知内容重写系统<br/>packages/editor/src/editor/rewriteUnknowContent.ts"]
RENDER["编辑器渲染器<br/>packages/editor/src/editor/render.tsx"]
...
subgraph "未知节点扩展"
UNKNODE["未知节点扩展<br/>packages/editor/src/extensions/unknown-node/unknown-node.ts"]
UNKVIEW["未知节点视图<br/>packages/editor/src/extensions/unknown-node/UnknowNodeView.tsx"]
end
subgraph "示例与集成"
...
- [packages/editor/src/editor/collaboration.tsx](file://packages/editor/src/editor/collaboration.tsx#L1-L209)
...
- [packages/editor/src/editor/rewriteUnknowContent.ts](file://packages/editor/src/editor/rewriteUnknowContent.ts#L1-L169)
- [packages/editor/src/editor/render.tsx](file://packages/editor/src/editor/render.tsx#L1-L141)
- [packages/editor/src/extensions/unknown-node/unknown-node.ts](file://packages/editor/src/extensions/unknown-node/unknown-node.ts#L1-L27)
- [packages/editor/src/extensions/unknown-node/UnknowNodeView.tsx](file://packages/editor/src/extensions/unknown-node/UnknowNodeView.tsx#L1-L26)
...
- **未知内容重写系统**：增强null安全检查，改进不受支持内容类型的回退机制，提高系统稳定性和可靠性。
...
## 架构总览
编辑器内核采用"扩展即插件"的理念，通过 AppContext 注入 PluginManager，useEditorExtension 统一解析内置与插件扩展，再交由 Tiptap 初始化编辑器。协作能力通过 Hocuspocus 提供者与 Yjs 文档实现，支持断线重连、感知用户状态与增量同步。新增的未知内容重写系统在编辑器初始化前对JSON内容进行null安全检查和类型验证，确保系统的稳定性和可靠性。
...
participant Rewriter as "未知内容重写器"
participant PM as "PluginManager"
participant Ctx as "AppContext"
UI->>Rewriter : rewriteUnknownContent(内容, Schema, 选项)
Rewriter->>Rewriter : null安全检查与类型验证
Rewriter->>Rewriter : 不受支持内容类型回退处理
Rewriter-->>UI : 返回清理后的JSON内容
...
- [packages/editor/src/editor/collaboration.tsx](file://packages/editor/src/editor/collaboration.tsx#L140-L149)
- [packages/editor/src/editor/rewriteUnknowContent.ts](file://packages/editor/src/editor/rewriteUnknowContent.ts#L131-L169)
- [packages/editor/src/editor/use-extension.ts](file://packages/editor/src/editor/use-extension.ts#L1-L63)
- [packages/common/src/core/PluginManager.ts](file://packages/common/src/core/PluginManager.ts#L1-L170)
...
participant Rewriter as "rewriteUnknownContent"
participant Ext as "useEditorExtension"
participant PM as "PluginManager"
participant Prov as "TiptapCollabProvider"
participant Y as "Y.Doc"
Page->>Rewriter : 处理JSON内容(增强null安全检查)
Rewriter-->>Page : 返回清理后的内容
...
CtxProv --> Rewriter["rewriteUnknownContent 进行null安全检查"]
Rewriter --> Resolve["useEditorExtension 解析扩展"]
...
- **未知节点扩展(unknown-node)**：处理不受支持的节点类型，提供友好的用户提示和市场链接
...
I["unknown-node 扩展"] --> J["UnknownNode 视图组件"]
J --> K["UnknownNodeView 提示界面"]
K --> L["市场链接与插件安装引导"]
```
...
### 系统概述
未知内容重写系统是编辑器内核的重要组成部分，专门用于处理不受支持的节点和标记类型，通过增强的null安全检查和智能回退机制，显著提高了系统的稳定性和可靠性。

### 核心功能特性
- **增强的null安全检查**：在处理JSON内容前进行全面的null和undefined检查，防止运行时错误
- **不受支持内容类型的智能回退**：将未知节点转换为unknownNode类型，保持文档结构完整性
- **详细的重写内容追踪**：记录所有被重写的节点和标记，便于调试和审计
- **灵活的回退策略**：支持将未知内容转换为段落或完全省略，满足不同使用场景

### 实现原理
系统通过递归遍历JSON内容树，对每个节点执行以下检查：

1. **标记过滤**：移除不受支持的标记，记录重写信息
2. **内容递归**：对嵌套内容进行深度优先遍历
3. **节点类型验证**：检查节点类型是否在有效节点集合中
4. **智能回退**：根据配置决定回退策略

### 使用场景
- **协作编辑器初始化**：在设置编辑器内容前进行内容重写
- **编辑器渲染器**：在创建编辑器实例时进行内容验证
- **插件兼容性**：处理第三方插件产生的未知内容类型

### 配置选项
- `fallbackToParagraph`：控制未知节点是否回退为段落，默认为true

**章节来源**
- [packages/editor/src/editor/rewriteUnknowContent.ts](file://packages/editor/src/editor/rewriteUnknowContent.ts#L1-L169)
- [packages/editor/src/editor/collaboration.tsx](file://packages/editor/src/editor/collaboration.tsx#L140-L149)
- [packages/editor/src/editor/render.tsx](file://packages/editor/src/editor/render.tsx#L60-L70)

## 依赖关系分析
- 导出入口统一导出编辑器相关类型、工具与提供者，便于上层直接使用
- 协作编辑器依赖扩展解析、上下文与样式主题，**新增依赖未知内容重写系统**
- useEditorExtension 依赖 AppContext 与 PluginManager，形成"插件驱动"的扩展装配链路
- **未知内容重写系统被协作编辑器和渲染器共同依赖**
...
COLLAB --> REWRITE["rewriteUnknowContent.ts"]
RENDER["render.tsx"] --> REWRITE
USEEXT --> PM["PluginManager.ts"]
USEEXT --> APPCTX["AppContext.ts"]
REWRITE --> UNKNODE["unknown-node.ts"]
UNKNODE --> UNKVIEW["UnknowNodeView.tsx"]
...
- **未知内容重写优化**：递归处理采用深度优先算法，时间复杂度为O(n)，其中n为内容节点数量
- **null安全检查成本**：在内容初始化时进行一次性检查，避免运行时频繁null检查带来的性能损耗
...
- **未知内容重写问题**
  - 检查rewriteUnknownContent函数的返回值，确认是否正确处理了null输入
  - 验证Schema对象是否正确初始化，确保有效节点和标记集合完整
  - 查看rewrittenContent数组，确认被重写的节点类型和原始内容
  - 参考路径：[未知内容重写实现](file://packages/editor/src/editor/rewriteUnknowContent.ts#L131-L169)
...
## 结论
本内核以 Tiptap/ProseMirror 为基础，结合 Yjs 与 Hocuspocus 实现高性能、低冲突的实时协作编辑；通过 AppContext 与 PluginManager 构建可插拔扩展生态；通过 PageContext 与 Provider 接口实现上下文与外部能力的解耦。新增的未知内容重写系统通过增强的null安全检查和智能回退机制，显著提高了系统的稳定性和可靠性，为复杂场景下的文档组织需求提供了坚实的技术保障。
...
- 集成第三方扩展
  ...
  - 参考路径：[AppContext 注入](file://packages/common/src/core/AppContext.ts#L1-L13), [协作编辑器使用扩展](file://packages/editor/src/editor/collaboration.tsx#L1-L209)
- **开发未知内容处理扩展**
  - 参考未知节点扩展的实现模式，处理不受支持的内容类型
  - 使用UnknownNode扩展作为基础，创建自定义的未知内容处理器
  - 参考路径：[未知节点扩展](file://packages/editor/src/extensions/unknown-node/unknown-node.ts#L1-L27), [未知节点视图](file://packages/editor/src/extensions/unknown-node/UnknowNodeView.tsx#L1-L26)
