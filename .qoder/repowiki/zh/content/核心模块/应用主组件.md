...
## 项目结构
应用主组件位于 packages/core/src，采用"入口组件 + 布局 + 菜单 + 引导流程 + 插件生态"的分层设计：
...
- [packages/core/src/components/SiderMenu.tsx](file://packages/core/src/components/SiderMenu.tsx#L1-L275)
- [packages/core/src/components/Welcome/index.tsx](file://packages/core/src/components/Welcome/index.tsx#L1-L245)
...
## 架构总览
应用采用"入口路由 + 动态插件 + 布局容器 + 引导流程"的架构模式：
...
**图表来源**
- [packages/core/src/App.tsx](file://packages/core/src/App.tsx#L60-L162)
...
**更新** 新增了'/welcome'路由，集成Welcome组件作为用户引导流程的一部分。
...
**更新** 新增了'/welcome'静态路由，为未登录用户提供引导流程。
...
**更新** 新增了Welcome组件的参与，为未登录用户提供引导流程。
...
## 结论
该应用主组件通过"入口路由 + 插件生态 + 布局容器 + 引导流程"的架构，实现了高扩展性的知识库管理平台。App.tsx 负责启动流程与路由构建，Layout 与 SiderMenu 提供一致的导航体验，Welcome 组件为新用户提供完整的引导流程，PluginManager 与 AppContext 保障插件的动态加载与菜单聚合。配合完善的事件总线与全局状态，系统具备良好的可维护性与可扩展性。
