...
- 移除了旧的bubble菜单系统组件
- **增强viewport边界处理逻辑**：优化评论卡片在各种屏幕尺寸下的显示，防止超出屏幕边缘
...
H --> Q[Action Controls]
H --> R[Viewport Boundary Handling]
end
```
...
**更新** 评论卡片组件现在具备了完善的viewport边界处理能力，确保在各种屏幕尺寸下都能正确显示：

- **固定宽度策略**: 新建卡片使用260px宽度，展开卡片使用260px宽度，折叠卡片使用240px宽度
- **最大宽度限制**: 使用`max-w-[calc(100vw-24px)]`确保卡片不会超出视口宽度
- **响应式布局**: 通过CSS calc函数动态计算最大宽度，适应不同屏幕尺寸
- **安全间距**: 左右两侧保留12px的安全间距，防止卡片贴边显示
...
**更新** CommentMarginPanel组件增强了viewport边界处理逻辑，防止面板超出屏幕范围：

#### 左边缘计算与边界约束

```mermaid
flowchart TD
A[计算理想左边缘] --> B{获取编辑器DOM位置}
B --> |成功| C[rect.right + 16]
B --> |失败| D[保持当前值]
C --> E[计算最大允许左边缘]
E --> F[window.innerWidth - CARD_WIDTH - VIEWPORT_PADDING]
G[设置面板左边缘] --> H[Math.min(idealLeft, maxLeft)]
```

**图表来源**
- [CommentMarginPanel.tsx:100-109](file://packages/plugin-comment/src/extension/menu/CommentMarginPanel.tsx#L100-L109)

#### 关键参数说明

- **CARD_WIDTH**: 260px（卡片宽度）
- **VIEWPORT_PADDING**: 12px（视口安全间距）
- **idealLeft**: rect.right + 16（理想左边缘位置）
- **maxLeft**: window.innerWidth - CARD_WIDTH - VIEWPORT_PADDING（最大允许左边缘）

#### 实时监听与响应

- **窗口大小变化**: 监听window.resize事件，实时重新计算面板位置
- **编辑器滚动**: 监听编辑器容器的scroll事件，确保面板跟随滚动
- **事务更新**: 监听编辑器的transaction事件，及时更新线程位置

**章节来源**
- [CommentMarginPanel.tsx:96-138](file://packages/plugin-comment/src/extension/menu/CommentMarginPanel.tsx#L96-L138)
...
- **viewport边界处理优化**: 使用requestAnimationFrame优化位置计算性能
...
5. **检查viewport边界**: 验证面板是否被正确限制在视口范围内
...
**更新** 如果遇到评论卡片超出屏幕的问题，请检查以下方面：

1. **窗口尺寸变化**: 确认window.resize事件监听器正常工作
2. **编辑器容器滚动**: 验证编辑器容器的scroll事件处理
3. **CSS宽度计算**: 检查max-w-[calc(100vw-24px)]是否正确应用
4. **边界参数设置**: 验证CARD_WIDTH和VIEWPORT_PADDING常量值
5. **requestAnimationFrame**: 确认位置更新使用了性能优化的动画帧
...
- **响应式设计**: 增强的viewport边界处理确保在各种屏幕尺寸下都能正确显示
...
- **支持折叠模式**: 减少界面拥挤，提升可读性
- **增强的交互设计**: 更直观的评论操作和管理
- **viewport边界处理**: 确保评论卡片在各种屏幕尺寸下都能正确显示
- **响应式布局**: 自适应不同设备和屏幕尺寸

该评论系统为知识管理平台提供了强大的协作能力，用户可以在文档中进行高效的讨论和知识分享。新的margin面板架构不仅提升了用户体验，还通过增强的viewport边界处理逻辑确保了在各种使用场景下的稳定性和可靠性，为未来的功能扩展奠定了坚实的基础。